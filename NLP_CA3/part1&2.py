...
class SRLModel(nn.Module):
    ...
    def forward(self, x, verb_indices, lengths):
        ...
        verb_hidden = lstm_out[range(len(lstm_out)), verb_indices]
        verb_hidden_expanded = verb_hidden.unsqueeze(1).expand(-1, lstm_out.shape[1], -1)
        combined_output = torch.cat((lstm_out, verb_hidden_expanded), dim=2)
...
valid_report = detailed_f1_scores(model, valid_loader, srl_data_handler.label_dict)
